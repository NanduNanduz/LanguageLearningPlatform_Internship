...
//----------------------------Get Route For Fetching Particular Instructor Details----------------------
...
//---------------------------------- Edit Course Details by instructor ------------------
...
// -------------------------------Adding New Course and Uploading Video With It By Instructor
  ...
  try {
    ...
    if (req.files.videos && req.files.videos.length > 0) {
      if (
        ...
        videoTitles.length !== req.files.videos.length
      ) {
        return res.status(400).json({
          success: false,
          message: "Each video must have a corresponding title",
        });
      }
...
//---------------------------------------Deleting Videos Inside a Course By Instructor-------------------
export const deleteVideoFromCourse = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Video deleted successfully", course });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};

//-----------------------------------------Editing Videos Inside a Course By Instructor---------------------------------------
export const updateVideoInCourse = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ success: false, message: "Invalid courseId or videoId" });
    }
    // Find the course
    const course = await courseModel.findById(courseId);
    if (!course) {
    ...
    if (videoIndex === -1) {
      return res
        .status(404)
        .json({ success: false, message: "Video not found in course" });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Video updated successfully", course });
  } catch (error) {
...
//-----------------------------Adding Videos And Assignments As PDF After Creating Course By Instructor-----------------------------
export const addVideosAndResources = async (req, res) => {
  try {
    ...
    if (req.files.videos && req.files.videos.length > 0) {
      if (
        !Array.isArray(videoTitle) ||
        videoTitle.length !== req.files.videos.length
      ) {
        return res.status(400).json({
          success: false,
          message: "Each video must have a corresponding title",
    ...
    if (req.files.resources && req.files.resources.length > 0) {
      if (
        ...
        resourceName.length !== req.files.resources.length
      ) {
        return res.status(400).json({
          success: false,
          message: "Each resource must have a corresponding name",
        });
      }
      resourceUploads = await Promise.all(
        req.files.resources.map(async (pdfFile, index) => {
    ...
    res.status(200).json({
      success: true,
      message: "Videos and resources added successfully",
      course,
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        ...
        .fontSize(14)
        .fillColor("#555")
        .text("Issued on: " + new Date().toDateString(), 0, 380, {
          align: "center",
        });
...
export const createQuizQuestions = async (req, res) => {
  try {
    ...
    const questions = [];
    const numQuestions = Array.isArray(questionTexts)
      ? questionTexts.length
      : 1;
    for (let i = 0; i < numQuestions; i++) {
      const questionText = Array.isArray(questionTexts)
        ? questionTexts[i]
        : questionTexts;
      const options = [
      ...
      const correctAnswerIndex = parseInt(
        Array.isArray(correctAnswers) ? correctAnswers[i] : correctAnswers,
        10
      );
      if (
        !questionText ||
        ...
        return res.status(400).json({
          success: false,
          message: `Invalid data for question ${i + 1}`,
        });
      }
...
export const getQuizzesByCourse = async (req, res) => {
  try {
    ...
    if (!quizzes.length) {
      return res
        .status(404)
        .json({ success: false, message: "No quizzes found for this course" });
    }
...
export const editQuizQuestion = async (req, res) => {
  try {
    ...
    if (questionIndex === -1) {
      return res
        .status(404)
        .json({ success: false, message: "Question not found in quiz" });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Question updated successfully", quiz });
  } catch (error) {
...
export const deleteResource = async (req, res) => {
  try {
    ...
    if (resourceIndex === -1) {
      return res
        .status(404)
        .json({ message: "Resource not found in the course" });
    }
    ...
    const enrolledStudents = await Promise.all(
      course.studentsEnrolled.map(async (student) => {
        const user = await userModel
          .findById(student.studentId._id)
