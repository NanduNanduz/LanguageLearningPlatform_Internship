...
export const getQuizByCourse = async (req, res) => {
  try {
    ...
    if (!quiz) {
      return res
        .status(404)
        .json({ success: false, message: "Quiz not found for this course" });
    }
...
export const updateVideoProgress = async (req, res) => {
  try {
    ...
    let enrolledStudent = course.studentsEnrolled.find(
    ...
    if (!enrolledCourse.completedVideos.includes(videoId)) {
      ...
      enrolledStudent.progressPercentage = progressPercentage;
...
export const getCourseProgress = async (req, res) => {
  try {
    ...
    if (!enrolledCourse) {
      return res
        .status(404)
        .json({ message: "Progress not found for this course" });
    }
    // Send progress data
    res.status(200).json({
...
export const checkRefundEligibility = async (req, res) => {
  try {
    const { userId, courseId } = req.params;
    //Find the user and their enrollment for the course
    ...
    //Check eligibility (20% < progress < 50%)
