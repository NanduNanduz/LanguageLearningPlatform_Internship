...
const StudentDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchStudentDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/student/studentDetails/${studentId}`);
        if (response.data) {
      ...
      <Card sx={{ boxShadow: 3, padding: 3 }}>
        <CardContent>
          ...
          <Grid container spacing={3} alignItems="center">
            <Grid item>
              <Avatar src={student.profilePicture} sx={{ width: 100, height: 100 }} />
            </Grid>
            <Grid item>
              <Typography variant="h4" fontWeight="bold">{student.name}</Typography>
              <Typography variant="body1" color="textSecondary">{student.email}</Typography>
              <Typography variant="body2" color="textSecondary">{student.qualification}</Typography>
            </Grid>
          </Grid>
          ...
          {assignmentSubmissions.length > 0 ? (
            assignmentSubmissions.map((assignment, index) => (
              <Paper key={index} sx={{ padding: 2, marginTop: 2 }}>
                <Typography variant="body1">
                  <strong>Assignment {index + 1}:</strong> {assignment.status}
                </Typography>
              </Paper>
            ))
          ) : (
